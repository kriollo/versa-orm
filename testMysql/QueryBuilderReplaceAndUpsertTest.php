...
class QueryBuilderReplaceAndUpsertTest extends TestCase
{
    ...
    public function test_replace_into_basic(): void
    {
        ...
        if (self::$config['driver'] !== 'mysql') {
            static::markTestSkipped('REPLACE INTO is only supported in MySQL');
        }
    ...
    public function test_replace_into_vs_upsert_behavior_difference(): void
    {
        // Solo ejecutar si estamos usando MySQL
        if (self::$config['driver'] !== 'mysql') {
            static::markTestSkipped('REPLACE INTO comparison is only relevant for MySQL');
        }
        ...
        static::assertNull($afterReplace['description']);
        static::assertNull($afterReplace['category']);
    ...
    public function test_large_dataset_performance(): void
    {
        // Solo ejecutar si estamos usando MySQL
        if (self::$config['driver'] !== 'mysql') {
            static::markTestSkipped('Performance test is MySQL specific');
        }
