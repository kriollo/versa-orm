...
/**
 * Tests para la creación de encabezados de factura con detalles en JSON y concatenación de fechas en MySQL.
 */
class MySqlInvoiceJsonTest extends TestCase
{
    ...
    public function test_invoice_header_with_items_json(): void
    {
        ...
        // En MySQL usamos JSON_ARRAYAGG y JSON_OBJECT
        $qb
            ->lazy()
        ...
        $qb->selectRaw(
            '(SELECT JSON_ARRAYAGG(JSON_OBJECT(\'id\',fi.id,\'tipo\',fi.tipo,\'descripcion\',fi.descripcion,\'cantidad\',fi.cantidad,\'precio_unitario\',fi.precio_unitario,\'subtotal\',fi.subtotal,\'detalles\',fi.detalles)) FROM factura_items fi WHERE fi.id_factura=facturas.id) AS items',
        );
        ...
        // MySQL retorna el JSON como array automáticamente si el cast funciona, o como string quecollect() decodifica
    ...
    public function test_date_concatenation_with_concat_function(): void
    {
        ...
        // Usando la función CONCAT de MySQL
    ...
    public function test_date_concatenation_with_pipe(): void
    {
        ...
        // MySQL no soporta || por defecto a menos que se cambie el sql_mode
        // Pero podemos probarlo si el entorno lo permite o simplemente asegurarnos de que la lógica de escape funcione
        $qb->select(['id', 'numero'])->selectRaw("CONCAT(anno, '|', mes) AS fecha_formateada");
