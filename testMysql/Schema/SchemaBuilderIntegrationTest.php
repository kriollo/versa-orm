...
/**
 * Tests de integración para SchemaBuilder con MySQL.
 */
class SchemaBuilderIntegrationTest extends TestCase
{
    ...
    public function testCreateTableWithEnumAndSet(): void
    {
        $this->schema->create('test_users', static function (Blueprint $table) {
            $table->id();
            $table->enum('status', ['active', 'inactive', 'suspended'])->default('active');
            $table->set('roles', ['admin', 'editor', 'viewer'])->nullable();
        });

        static::assertTrue($this->schema->hasTable('test_users'));
        static::assertTrue($this->schema->hasColumn('test_users', 'status'));
        static::assertTrue($this->schema->hasColumn('test_users', 'roles'));
    }
    ...
    public function testCreateTableWithFullTextIndex(): void
    {
        $this->schema->create('test_posts', static function (Blueprint $table) {
            ...
            $table->string('title', 200);
            $table->text('content');
            $table->fullText(['title', 'content'], 'ft_search');
        });

        static::assertTrue($this->schema->hasTable('test_posts'));

        $indexes = $this->schema->getIndexes('test_posts');
        static::assertNotEmpty($indexes);
    }
