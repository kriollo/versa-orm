...
class VectorSupportTest extends TestCase
{
    ...
    public function test_can_create_table_with_vector_column(): void
    {
        ...
        $this->assertNotEmpty($result, 'La columna embedding debe existir en information_schema');

        $row = $result[0];
        // pgvector se registra como tipo USER-DEFINED cuyo udt_name es 'vector'
        $this->assertSame('vector', strtolower((string) ($row['udt_name'] ?? '')));
    }
    ...
    public function test_can_create_table_with_tsvector_column(): void
    {
        ...
        $this->assertNotEmpty($result, 'La columna search_vector debe existir');
        $this->assertSame('tsvector', strtolower((string) ($result[0]['data_type'] ?? '')));
    }
    ...
    public function test_vector_column_accepts_embeddings(): void
    {
        ...
        $this->assertSame('Doc A', $row['title']);
    }
    ...
    public function test_cosine_similarity_returns_correct_order(): void
    {
        ...
        $query  = [0.9, 0.1, 0.0];
        $result = self::$orm->table(self::TABLE)
        ...
        $this->assertSame('Doc A', $result[0]['title']);
    }
