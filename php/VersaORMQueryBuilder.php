...
class VersaORMQueryBuilder
{
     ...
     *
     * @return VersaORMModel
     */
    public function dispense(): VersaORMModel
    {
        return new VersaORMModel($this->table, $this->config);
    }
    ...
    private function executeRustCommand(string $action, array $params)
    {
        ...
        $binaryPath = __DIR__ . '/../versaorm_cli/target/release/versaorm';
        $command = sprintf('%s %s', $binaryPath, escapeshellarg($payload));
        $output = shell_exec($command);

        if ($output === null) {
            throw new \Exception('Failed to execute the VersaORM binary.');
        }
