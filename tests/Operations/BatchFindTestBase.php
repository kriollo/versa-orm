...
trait BatchFindTestBase
{
    public function testFindInBatchesBasic(): void
    {
        ...
        VersaModel::findInBatches(
            'users',
            static function (array $models) use (&$batches, &$invocations): void {
                $invocations++;
    ...
    public function testFindInBatchesWithCondition(): void
    {
        ...
        VersaModel::findInBatches(
            'users',
            static function (array $models) use (&$ids, &$invocations): void {
                $invocations++;
                foreach ($models as $m) {
                    $ids[] = (int) $m->id;
                }
    ...
    public function testFindInBatchesWithLargeBatchSize(): void
    {
        ...
        VersaModel::findInBatches(
            'users',
            static function (array $models) use (&$count, &$ids): void {
                $count++;
                foreach ($models as $m) {
                    $ids[] = (int) $m->id;
                }
