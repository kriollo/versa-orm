...
class PHPVersionCompatibilityTest extends TestCase
{
    ...
    public function testBasicVersaORMInstantiation(): void
    {
        ...
        self::assertInstanceOf(VersaORM::class, $orm);

        // Test basic functionality
        try {
            $orm->exec('SELECT 1');
            self::assertTrue(true, 'Database connection works');
        } catch (Exception $e) {
    ...
    public function testPHP74SpecificFeatures(): void
    {
        ...
        self::assertTrue(self::$versionFeatures['typed_properties']);

        // Test arrow functions (PHP 7.4+)
        self::assertTrue(self::$versionFeatures['arrow_functions']);

        // Test null coalescing assignment operator (PHP 7.4+)
        self::assertTrue(self::$versionFeatures['null_coalescing_assignment']);

        // Test spread operator in array expressions (PHP 7.4+)
        self::assertTrue(self::$versionFeatures['array_spread']);
    }
    ...
    public function testPHP80SpecificFeatures(): void
    {
        ...
        self::assertTrue(self::$versionFeatures['named_arguments']);

        // Test attributes (PHP 8.0+)
        self::assertTrue(self::$versionFeatures['attributes']);

        // Test constructor property promotion (PHP 8.0+)
        self::assertTrue(self::$versionFeatures['constructor_promotion']);

        // Test union types (PHP 8.0+)
        self::assertTrue(self::$versionFeatures['union_types']);

        // Test match expression (PHP 8.0+)
        self::assertTrue(self::$versionFeatures['match_expression']);

        // Test nullsafe operator (PHP 8.0+)
        self::assertTrue(self::$versionFeatures['nullsafe_operator']);
    }
    ...
    public function testPHP81SpecificFeatures(): void
    {
        ...
        // Test enums (PHP 8.1+)
        self::assertTrue(self::$versionFeatures['enums']);

        // Test readonly properties (PHP 8.1+)
        self::assertTrue(self::$versionFeatures['readonly_properties']);

        // Test intersection types (PHP 8.1+)
        self::assertTrue(self::$versionFeatures['intersection_types']);

        // Test first-class callable syntax (PHP 8.1+)
        self::assertTrue(self::$versionFeatures['first_class_callables']);

        // Test new in initializers (PHP 8.1+)
        self::assertTrue(self::$versionFeatures['new_in_initializers']);
    }
    ...
    public function testPHP82SpecificFeatures(): void
    {
        ...
        self::assertTrue(self::$versionFeatures['readonly_classes']);

        // Test DNF types (PHP 8.2+)
        self::assertTrue(self::$versionFeatures['dnf_types']);

        // Test constants in traits (PHP 8.2+)
        self::assertTrue(self::$versionFeatures['constants_in_traits']);
    }
    ...
    public function testPHP83SpecificFeatures(): void
    {
        ...
        self::assertTrue(self::$versionFeatures['typed_class_constants']);

        // Test dynamic class constant fetch (PHP 8.3+)
        self::assertTrue(self::$versionFeatures['dynamic_class_constant_fetch']);

        // Test override attribute (PHP 8.3+)
        self::assertTrue(self::$versionFeatures['override_attribute']);
    }
    ...
    public function testErrorHandlingByVersion(): void
    {
        ...
        try {
            $orm->exec('SELECT 1');
            self::assertTrue(true, 'Basic operation succeeded');
        } catch (Exception $e) {
