...
class QueryBuilderJoinRawTest extends TestCase
{
    ...
    {
        if (self::$orm !== null) {
            self::$orm->exec('PRAGMA foreign_keys = OFF');
            self::$orm->exec('DROP TABLE IF EXISTS anima_campana_images');
            self::$orm->exec('DROP TABLE IF EXISTS anima_videos');
            self::$orm->exec('DROP TABLE IF EXISTS anima_campanas');
            self::$orm->exec('DROP TABLE IF EXISTS versa_users');
    ...
    public function testCompleteUserCase(): void
    {
        ...
        $qb->leftJoin('anima_campana_images', 'anima_campana_images.id_campana', '=', 'anima_campanas.id')->onRaw(
            'anima_campana_images.is_default = 1',
        );
    ...
    /**
     * Test de rightJoinRaw - SQLite no soporta RIGHT JOIN nativamente
     * Se emula con LEFT JOIN invertido.
     */
    public function testRightJoinRaw(): void
    {
        // SQLite no soporta RIGHT JOIN, el ORM lo emula
        $result = self::$orm
            ->table('anima_campanas')
    ...
    public function testMultipleLeftJoinRaw(): void
    {
        $result = self::$orm
            ...
            ->leftJoinRaw(
                'anima_campana_images ON anima_campana_images.id_campana = anima_campanas.id AND anima_campana_images.is_default = 1',
            )
            ->leftJoinRaw(
                '(SELECT id_campana, COUNT(*) as video_count FROM anima_videos GROUP BY id_campana) as video_counts ON video_counts.id_campana = anima_campanas.id',
            )
    ...
    {
        // Tabla de campañas (similar al caso del usuario) - SQLite
        self::$orm->exec('DROP TABLE IF EXISTS anima_campana_images');
        self::$orm->exec('DROP TABLE IF EXISTS anima_videos');
        self::$orm->exec('DROP TABLE IF EXISTS anima_campanas');
        self::$orm->exec('DROP TABLE IF EXISTS versa_users');
        ...
        self::$orm->exec('
            CREATE TABLE anima_campanas (
                ...
                id_user INTEGER,
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (id_user) REFERENCES versa_users(id)
            )
        ');
        ...
        self::$orm->exec('
            CREATE TABLE anima_campana_images (
                ...
                created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
                FOREIGN KEY (id_campana) REFERENCES anima_campanas(id) ON DELETE CASCADE
            )
        ');
    ...
    private static function insertTestData(): void
    {
        ...
        self::$orm->exec("
            ...
            (1, '/images/campana1_default.jpg', 1),
            (1, '/images/campana1_alt.jpg', 0),
            (2, '/images/campana2_default.jpg', 1),
            (3, '/images/campana3_alt.jpg', 0)
        ");
